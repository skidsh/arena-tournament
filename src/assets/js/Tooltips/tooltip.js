...
if (typeof $CoTTooltip == "undefined") {
      ...
      function init()
  {
          if (external)
    {
              ...
              script.src = "./assets/js/Tooltips/global.js";
    ...
    var	url = "//" + xpac + $CoT.expUrl + "/";
          ...
          if (!$CoT.fullIdList[id]) {
            $CoT.fullIdList[id] = {};
          }
          if (!$CoT.fullIdList[id][fullId])
          {
            $CoT.fullIdList[id][fullId] = true;
          }
              ...
              if (currentParams != null) {
                  ...
                  if (currentParams.pcs && currentParams.pcs.length) {
                      ...
                      for (var i = 0, len = currentParams.pcs.length; i < len; ++i) {
                          var m;
                          if (m = html.match(new RegExp('<span><a href="?item=([0-9]+)*' + currentParams.pcs[i] + '(:[0-9]+)*">(.+?)</a></span>'))) {
                              html = html.replace(m[0], '<span class="q8"><!--si' + currentParams.pcs[i] + '--><a href="?item=' + currentParams.pcs[i] + '">' + m[3] + "</a></span>");
                              ++n;
                          }
                      }
    ...
    */
      ...
      function getFullId(id, params, expansion) {
          var fullId = id + (params.rand ? "r" + params.rand : "") + (params.ench ? "e" + params.ench : "") + (params.gems ? "g" + params.gems.join(",") : "") + (params.sock ? "s" : "");
          return fullId;
      }
  /*
  ...
  */
    ...
    /* TODO */
      ...
      $CoT.IsQueryPending = function(id) {
        for (i in $CoT.fullIdList[id]) {
          if ($CoT.fullIdList[id][i]) {
            return true;
          }
        }
      }

      this.findNextFullId = function(id) {
        var fullId;
        for (i in $CoT.fullIdList[id]) {
          if ($CoT.fullIdList[id][i]) {
            $CoT.fullIdList[id][i] = false;
            fullId = i;
            break;
          }
        }
      ...
      this.regNpc = function (id, locale, json, expansion) {
          this.register(TYPE_NPC, this.findNextFullId(id), locale, json, expansion);
      };

      this.regObject = function (id, locale, json, expansion) {
          this.register(TYPE_OBJECT, this.findNextFullId(id), locale, json, expansion);
      };

      this.regItem = function (id, locale, json, expansion) {
          this.register(TYPE_ITEM, this.findNextFullId(id), locale, json, expansion);
      };

      this.regQuest = function (id, locale, json, expansion) {
          this.register(TYPE_QUEST, this.findNextFullId(id), locale, json, expansion);
      };

      this.regSpell = function (id, locale, json, expansion) {
          this.register(TYPE_SPELL, this.findNextFullId(id), locale, json, expansion);
      };

      this.regAchievement = function (id, locale, json, expansion) {
          this.register(TYPE_ACHIEVEMENT, this.findNextFullId(id), locale, json, expansion);
      };
